...
export default function Navbar() {
  ...
  if (size.width > 1500) {
    return (
      <header className="navbar w-1/2 z-50 text-black fixed top-0 left-0 right-0 mx-auto h-36">
          ...
          <ul className="font-medium flex flex-row justify-end pl-2 space-x-4">
            {navItems.map(item => (
              <li key={item.key} >
                <Link
                  href={item.href}
                  className="transition flex flex-row space-x-1 bg-button-teal border-2 max-w-32 min-w-fit border-white rounded-3xl p-4 hover:text-black hover:bg-slate-100 hover:shadow-xl hover:-translate-y-1 hover:scale-110"
                >
                  {item.icon}
